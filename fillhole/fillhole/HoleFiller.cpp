...
void HoleFiller::Run(string offfile, std::string offoutfile)
{
	Mesh * m = new Mesh();
	m->loadOff((char *)offfile.c_str());

	HoleFiller hFiller(*m);
	hFiller.fillHoles();

	m->writeOff((char *)offoutfile.c_str());
}
...
void HoleFiller::fillHoles()
{
	...
	cout << "trianglulateHoles holes..." << endl;
	trianglulateHoles();

	cout << "refining holes..." << endl;
